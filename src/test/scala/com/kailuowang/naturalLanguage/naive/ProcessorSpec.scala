...
class ProcessorSpec extends Specification {
  implicit def toStream(str: String) = str.toStream
  ...
  "tokenize" should {
    "return empty stream when string is empy" in {
  ...
  "breakToSentences" should {
    ...
    "works with one character" in {
      ...
      Processor.splitToSentences(input).map(mkString(_)) must equalTo(Stream("a"))
    }
